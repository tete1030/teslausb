...
function nm_get_wifi_client_device () {
  for i in {1..5}
  do
    WLAN="$(nmcli -t -f TYPE,DEVICE d | grep 'wifi:' | grep -v ":ap0$" | cut -d':' -f 2)"
    if [ -n "$WLAN" ]
    then
      break;
    fi
...
EOF
  chmod a+x /root/bin/setup_ap.sh || return 1
  echo 'ACTION=="add", SUBSYSTEM=="net", KERNEL=="'$WLAN'", RUN+="/root/bin/setup_ap.sh"' > /etc/udev/rules.d/90-teslausb-ap.rules
...
then
  ...
  apt-get -y --force-yes install iw dnsmasq || return 1
