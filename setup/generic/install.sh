...
then
  ...
  then
    ...
    cat <<- EOF > /etc/rc.local
		#!/bin/bash
		{
		  while ! curl -s https://raw.githubusercontent.com/tete1030/teslausb/main-dev/setup/generic/install.sh
		  do
		    sleep 1
		  done
		} | bash
    ...
    {
      while ! curl -s https://raw.githubusercontent.com/tete1030/teslausb/main-dev/tools/debian-resizefs.sh
      do
        sleep 1
      done
    } | bash -s 3G
...
then
  while ! curl -o /teslausb/teslausb_setup_variables.conf https://raw.githubusercontent.com/tete1030/teslausb/main-dev/pi-gen-sources/00-teslausb-tweaks/files/teslausb_setup_variables.conf.sample
  do
    sleep 1
  done
fi
...
then
  while ! curl -o /teslausb/wpa_supplicant.conf.sample https://raw.githubusercontent.com/tete1030/teslausb/main-dev/pi-gen-sources/00-teslausb-tweaks/files/wpa_supplicant.conf.sample
  do
    sleep 1
  done
fi
...
rm -f /etc/rc.local
while ! curl -o /etc/rc.local https://raw.githubusercontent.com/tete1030/teslausb/main-dev/pi-gen-sources/00-teslausb-tweaks/files/rc.local
do
  sleep 1
done
chmod a+x /etc/rc.local
...
then
  cat <<- EOF >> /root/.bashrc
	...
	then
	  ...
	  echo "| https://github.com/tete1030/teslausb/blob/main-dev/doc/OneStepSetup.md  |"
